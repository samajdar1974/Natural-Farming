...
def translate_text(text, source_lang='en', target_lang='bn'):
    """
    Translates text from English to Bengali using AI4Bharat translation API.
    """
    api_url = "https://ai4bharat.iitm.ac.in/translate/IndicTransv2"
    payload = {
        "text": text,
    ...
    "Gangetic Plains": {
        "Field Crops": {
            "Jute": {
                ...
                "Land Preparation": translate_text("Fine tilth with 3-4 plowings; apply Jeevamrit (200 liters/acre)."),
                "Seed Treatment": translate_text("Use Beejamrit (5 liters solution per acre)."),
                "Water Management": translate_text ("Maintain proper drainage to prevent waterlogging."),
                "Weed Control": translate_text ("Manual weeding at 20 and 40 days."),
                "Fertilization": translate_text ("Apply Jeevamrit (200 liters/acre) every 15 days."),
                "Pest and Disease Management": translate_text ("Use Neem Oil (5 ml/liter) every 15 days.Use Neemastra (100 liters/acre) to control borers and aphids. Apply Agniastra (3-4 liters/100 liters of water) and Bramhastra (2 liters/100 liters of water) for additional pest control"),
                "Harvesting": translate_text("Harvest at pod maturity stage and dry before storage."),
...
API_KEY = "M8MUEBM5JPV49ZRBXNU2ECDJQ"
BASE_URL = "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline"


def get_weather_data(location, days=14):
    """
    Fetch weather data for the current and upcoming 7 days using Visual Crossing API.
    """
    url = f"{BASE_URL}/{location}?unitGroup=metric&include=current,days&key={API_KEY}&contentType=json"
    response = requests.get(url)
    if response.status_code == 200:
        return response.json()
    elif response.status_code == 403:
        st.error("ত্রুটি ৪০৩: অনুমতি নেই। দয়া করে আপনার API কী বা ব্যবহারের সীমা পরীক্ষা করুন।")
    else:
        st.error(f"আবহাওয়া তথ্য আনতে সমস্যা: {response.status_code} - {response.reason}")
    return None


def translate_condition(condition):
    """
    Translate weather condition names to Bengali.
    """
    translations = {
        "Clear": "পরিষ্কার",
        "Partially cloudy": "আংশিক মেঘলা",
        "Rain": "বৃষ্টি",
        "Rain, Partially cloudy": "বৃষ্টি, আংশিক মেঘলা",
        "Cloudy": "মেঘলা",
        "Overcast": "আবৃত"
    }
...
def display_current_weather(data):
    ...
    if data and "currentConditions" in data:
        ...
        st.write("### বর্তমান আবহাওয়া")
        st.write(f"**তাপমাত্রা:** {current['temp']}°C")
        st.write(f"**আবহাওয়া:** {translate_condition(current['conditions'])}")
        st.write(f"**আর্দ্রতা:** {current['humidity']}%")
        st.write(f"**বাতাসের গতি:** {current['windspeed']} কিমি/ঘণ্টা")
        st.write(f"**বৃষ্টিপাত:** {float(current.get('precip', 0) or 0):.1f} মিমি")

        st.write(f"**সূর্যোদয়:** {current.get('sunrise', 'N/A')}")
        st.write(f"**সূর্যাস্ত:** {current.get('sunset', 'N/A')}")
        st.write("---")
...
def display_forecast_data(data):
    """
    Display 15-day weather forecast in a table, starting from the next day of the current data's date.
    """
    if data and "days" in data:
        st.write("### ১৪ দিনের আবহাওয়ার পূর্বাভাস")

        # Convert the current date into a datetime object
        today = datetime.strptime(data["days"][0]["datetime"], "%Y-%m-%d")
        ...
        for day in data["days"]:
            # Include only data from the next day onward
            if day["datetime"] >= next_day:
                date = datetime.strptime(day["datetime"], "%Y-%m-%d").strftime("%d-%m-%Y")
                max_temp = f"{day['tempmax']:.1f}"
                min_temp = f"{day['tempmin']:.1f}"
                condition = translate_condition(day["conditions"])
                humidity = f"{day['humidity']:.1f}"
                wind_speed = f"{day['windspeed']:.1f}"
                rainfall = f"{float(day.get('precip', 0) or 0):.1f}"  # Default to 0 if no rainfall data

                weather_data.append({
                    "তারিখ": date,
                    "আবহাওয়া": condition,
                    "সর্বোচ্চ তাপমাত্রা (°C)": max_temp,
                    "সর্বনিম্ন তাপমাত্রা (°C)": min_temp,
                    "আর্দ্রতা (%)": humidity,
                    "বাতাসের গতি (কিমি/ঘণ্টা)": wind_speed,
                    "বৃষ্টিপাত (মিমি)": rainfall
                })
...
def main():
    ...
    if page == "Home":
        ...
        st.image("https://example.com/home_banner.jpg", use_container_width=True)  # Replace with your home banner image URL
        st.write(
            """
        ...
        st.title("বর্তমান আবহাওয়া ও ১৪ দিনের পূর্বাভাস")

        # JavaScript to fetch user's geolocation
        location_script = """
                <script>
                ...
                <input type="text" id="location-coords" name="location-coords" style="display:none;">
            """
        st.components.v1.html(location_script, height=0)

        # Input box for location
        location = st.text_input("অবস্থান:", "উত্তর ২৪ পরগণা")
        coords = st.query_params.get("location-coords", [""])[0]

        if coords:
            location = coords  # Use latitude,longitude if available

        if st.button("আবহাওয়া দেখুন"):
            with st.spinner("আবহাওয়ার তথ্য সংগ্রহ করা হচ্ছে..."):
                weather_data = get_weather_data(location)
                if weather_data:
                    display_current_weather(weather_data)

                    display_forecast_data(weather_data)
        ...
        if "Image" in input_details:
            st.image(input_details["Image"], caption=input_name, use_container_width=True)
    elif page == "Contact":
        st.header("Contact")
        st.write("""Head""")
